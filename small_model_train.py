...
X_work, Y_work, X_week, Y_week = small_reader('10.130.2.1')
X, Y = X_work, Y_work


if figShow:
    X_plotter_2D(day, X_work)
    X_plotter_2D(day, np.expand_dims(Y_work, axis=1))
...
model = trainable_small_model(Tx=Tx, LSTM_cell=LSTM_cell, densor=densor, reshaper=reshaper)
...
history = model.fit([np.array(X), a0, c0], Y, epochs=epochs, verbose=verbose)
...
uni_input_inference_model_test.save("saved_models/test_small_models/uni_input_inference_model_test.h5")
print("written to saved_models/test_small_models/uni_input_inference_model_test.h5")
...
multi_input_sequence_inference_model_test.save(f"saved_models/test_small_models/model{outputs}Inputs.h5")
print(f"written to saved_models/test_small_models/model{outputs}Inputs.h5")
...
for number_of_inputs in range(1, maxInput + 1):
    ...
    model.save(f"saved_models/small_models/model{number_of_inputs}Inputs.h5")
    print(f"written to saved_models/small_models/model{number_of_inputs}Inputs.h5")


maxInput = inputs
for number_of_inputs in range(1, maxInput + 1):
    model = multi_input_HiddenCell_sequence_inference_model(LSTM_cell, densor, reshaper, Ty=1, Tx=number_of_inputs)
    model.save(f"saved_models/hc_small_models/model{number_of_inputs}Inputs_HC.h5")  # hc means HiddenCell
    print(f" written to saved_models/hc_small_models/model{number_of_inputs}Inputs_HC.h5")
